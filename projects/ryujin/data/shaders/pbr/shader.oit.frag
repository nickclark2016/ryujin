...
layout (set = 2, binding = 2, r32ui) uniform uimage2D counter;
...
vec4 get_color()
{
    ...
    else
    {
        ...
        return vec4(color, baseAlbedo.a);
    }
}

void main(void)
{
    const uint offset = imageAtomicAdd(counter, ivec2(0), 1) + 1;
    const vec4 color = get_color();
    if (offset >= samplesPerPixel)
    {
        // tail blend?
    ...
    const uint prevOffset = imageAtomicExchange(imageAux, coord, offset);
    const vec4 sRGBColor = unPremultLinearToSRGB(color);

    const uvec4 stored = uvec4(packUnorm4x8(sRGBColor), floatBitsToUint(gl_FragCoord.z), 0, prevOffset);
    imageStore(aBuffer, int(offset), stored);
