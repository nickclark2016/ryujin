...
namespace ryujin
{
    ...
    void opaque_pbr_pass::render(graphics_command_list& cmd, buffer& indirect, buffer& count, std::size_t offset, std::size_t numBufferGroups)
    {
        cmd.bind_graphics_pipeline(_shader);

        const auto countPtr = reinterpret_cast<std::uint32_t*>(count.info.pMappedData) + offset;
        const auto indirectBaseAddr = offset * sizeof(gpu_indirect_call);
        auto indirectAddrOffset = 0;
        const auto countBaseAddr = offset * sizeof(std::uint32_t);
