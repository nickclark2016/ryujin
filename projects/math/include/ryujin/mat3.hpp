...
namespace ryujin
{
    ...
    struct alignas(sizeof(vec3<T>)) mat3
    {
        ...
        constexpr mat3(const T m00, const T m10, const T m20, const T m01, const T m11, const T m21, const T m02, const T m12, const T m22);

        constexpr vec3<T>& operator[](const sz index) noexcept;
        constexpr const vec3<T>& operator[](const sz index) const noexcept;
    ...
    template<numeric T>
    inline constexpr mat3<T>::mat3(const T m00, const T m10, const T m20, const T m01, const T m11, const T m21, const T m02, const T m12, const T m22)
        : m00(m00), m10(m10), m20(m20), m01(m01), m11(m11), m21(m21), m02(m02), m12(m12), m22(m22)
    {
    }
    
    template<numeric T>
    inline constexpr vec3<T>& mat3<T>::operator[](const sz index) noexcept
    {
        return columns[index];
    }
    
    template<numeric T>
    inline constexpr const vec3<T>& mat3<T>::operator[](const sz index) const noexcept
    {
        return columns[index];
    }
